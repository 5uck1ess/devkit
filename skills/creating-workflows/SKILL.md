...
- **expect** — Only for `command` steps. Values: `success` (default — non-zero exit is informational) or `failure` (step fails if exit code is 0). Enables bugfix reproduction gates: repro must fail before fix, pass after.
