...
4. **Dispatch in parallel** — Launch all available agents concurrently. Pass diff inline in each prompt (worktree agents can't see latest commits). Claude uses native background agent; Codex/Gemini use plugin (preferred) or CLI fallback.
